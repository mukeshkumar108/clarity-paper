...
The synthesis prompt produces a **short conversational reply** (2-3 paragraphs) plus **pathway cards** — curated exploration directions. This is NOT an essay or a report format.

**Hard rules (see `PROMPTS.md`):**
...
- **First sentence must be a specific finding or position** — never setup, never hedge
- **Reply is 2-3 paragraphs max** — not a 5-section essay
- **Pathways are the primary interaction model** — follow-up chips are secondary

**Pathways** are curated exploration directions that emerge from the evidence:
- Each has a `label` (short conversational title), `preview` (one-sentence teaser), `question` (the follow-up query it triggers), `evidenceFit`, `relevantPaperCount`, and `icon`
- At least one pathway must be based on `direct` evidence
- Pathways are generated by both the editorial model (as part of synthesis) and the follow-up model
- Icons: strong, complicated, population, emerging, practical, mechanism, contradiction
...
| Component | Role |
|-----------|------|
| `SynthesisAnswer` | Conversational reply — short first read (2-3 paragraphs), not a 5-section essay |
| `PathwayGroup` / `PathwayCard` | Curated exploration directions — primary interaction model for drilling deeper |
| `CompactEvidence` | Collapsible evidence shape (counts) + claim-level provenance |
| `EvidencePanel` | Claim-level provenance — expandable rows with verbatim abstract snippets and support labels |
| `FollowUpOptions` | Secondary quick-question chips (shown alongside but subordinate to pathway cards) |
| `MainRefineInput` | Free-text input for follow-up questions |
| `PapersSidebar` | Right sidebar — turn-grouped papers, deep-read, doi links |
| `FilterChips` | Quick filter pills (All / RCTs & meta-analyses / Human only) |
| `ChatCanvas` | Main two-column layout: left column (conversational flow) + right column (papers sidebar) |
